class CreateVersionCollections < ActiveRecord::Migration[6.1]
  def change
    ...
    user_ids.each do |uid|
      u = User.find(uid)
      sys_ready_from = Collection.find_or_create_by(user_id: chemotion_user.id, label: 'New Versions from', is_locked: true, is_shared: false)
      sys_ready_collection = Collection.create(user: chemotion_user, label: 'New Versions', ancestry: "#{sys_ready_from.id}")
