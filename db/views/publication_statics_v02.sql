...
union
select 'sample'::text AS el_type, 'xvial'::text AS ex_type, count(publications.id) AS e_cnt from publications
  join element_tags et on et.taggable_type = 'Sample' and (et.taggable_data -> 'xvial' is not null and et.taggable_data -> 'xvial' ->> 'num' != '')
  and et.taggable_id = publications.element_id
