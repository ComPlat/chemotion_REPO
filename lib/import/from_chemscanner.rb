...
module Import
  ...
  class FromChemscanner
    ...
    def import
      @cs_mids.each { |id| import_molecule(id) }
    ...
    def import_molecule(cs_mid)
      ...
      sample = Sample.find_by_id(cs_mol.imported_id)
      return sample unless sample.nil?

      molecule = Molecule.find_or_create_by_molfile(cs_mol.mdl)
      return if molecule.nil?

      sample = Sample.create

      sample_svg = Chemotion::RdkitService.svg_from_molfile(cs_mol.mdl)
    ...
    class << self
      def from_files_and_schemes(file_ids, scheme_ids, uid, collection_id, maintain = false)
        ...
        importers = from_schemes(scheme_ids, uid, collection_id, maintain)
      ...
      handle_asynchronously :from_files_and_schemes
