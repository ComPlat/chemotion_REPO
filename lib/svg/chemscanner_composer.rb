...
module SVG
  ...
  class ChemscannerComposer
    ...
    def self.reaction_svg_from_mdl(mdl_info, options = {})
      files = {
        starting_materials: [],
        products: [],
        solvents: [],
      ...
      %w[starting_materials reactants solvents products].each do |g|
        svg_arr = mdl_info[g.to_sym].map do |m, _|
          Chemotion::RdkitService.svg_from_molfile(m)
        end

        files[g.to_sym].concat(svg_arr)
      end

      new(files, options).compose_reaction_svg
    end
    ...
    def compose_reaction_svg
      ...
      y_center = if check_middle
                   middle_height
                 else
                   (@reactants_max_height * REACTANT_SCALE)
                 end
      ...
      reactant_svg, reactant_width = compose_material_group(
        ...
        y_center: y_center - (@reactants_max_height * REACTANT_SCALE / 2) - 5,
        is_reactants: true
      )
      ...
      solvent_svg, solvent_width = compose_material_group(
        ...
        start_at: @viewbox_width,
        scale: REACTANT_SCALE,
        y_center: y_center + (@solvents_max_height * REACTANT_SCALE / 2) + 10,
        is_reactants: true,
        is_solvents: true
      )
      ...
      arrow_svg = [
        "<g transform='translate(#{@viewbox_width}, #{y_center})'>",
        arrow_it,
        '</g>'
      ].join
    ...
    def init_parameters(options)
      ...
      @box_width = options[:width] || 1500
      @box_height = options[:width] || 440
    end
    ...
    def compose_material_group(materials, options = {})
      ...
      materials.each_with_index do |material, idx|
        if idx.positive?
          group_width += 10
          group_svg += divide_it(group_width, y_center)
          group_width += 50
        end
        ...
        y_shift = (y_center - viewbox[3] * scale / 2).round
