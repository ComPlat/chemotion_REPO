  ...
  module Docx
    class DetailSample < Detail
      ...
      def init_item
        ...
        has_description = obj.reactions.first&.description && obj.reactions.first.description["ops"]&.present?
        item += obj.reactions.first.description["ops"].map(&:to_h) if need_rxn_desc && has_description
        item
      end
    end
  end
