...
export default class GenericElementsAdmin extends React.Component {
  ...
  handleCreateLayer(_layer) {
    ...
    notification({ title: `Layer [${layer.key}]`, lvl: 'info', msg: 'This new layer is kept in the workspace temporarily. Please remember to press Save when you finish the editing.' });
  ...
  handleUpdateLayer(layerKey, updates) {
    ...
    && element.properties_template.layers[layerKey];
    let msg = 'The updates of this layer is kept in the workspace temporarily. Please remember to press Save when you finish the editing.';
    layer = { ...layer, ...updates };
    // reset cond_fields of fields is the layer is workflow
    if (layer.wf) {
      msg = `${msg} For a Workflow layer, the Field Restrict Setting is only applicable on its own layer.`;
      (layer.fields || []).map((f) => {
        const newF = f;
    ...
    element.properties_template.layers[`${layer.key}`] = layer;
    notification({ title: `Layer [${layer.key}]`, lvl: 'info', msg });
  ...
  handleActivateKlass(e) {
    ...
    GenericElsFetcher.deActivateKlass({ id: e.id, is_active: !e.is_active, klass: 'ElementKlass' })
      .then((result) => {
        if (result.error) {
          notification({ title: `${act} Element failed`, lvl: 'error', msg: result.error });
        } else {
          notification({ title: `${act} Element [${result.name}] successfully`, lvl: 'info', msg: `Element is ${act.toLowerCase()} now` });
          this.closeModal(this.fetchElements);
        }
      });
  }
  ...
  fetchElements() {
    GenericElsFetcher.fetchElementKlasses()
      .then((result) => {
        if (typeof result !== 'undefined' && typeof result.klass !== 'undefined') {
            ...
            <Panel>
              <Panel.Heading>
                ...
                <span className="button-right" >
                  ...
                  <GenButtonTooltip txt="Upload template" tip="Upload Element template in JSON format" fnClick={() => this.handleShowState('modal', 'Upload')} place="top" fa="fa-upload" />&nbsp;
  ...
  render() {
    ...
    const sortedLayers = layer.wf ?
      (sortBy(layers, l => l.position) || []).filter(l => l.key === layer.key) :
      (sortBy(layers, l => l.position) || []);
    return (
...
const GenericElementsAdminDnD = DragDropContext(HTML5Backend)(GenericElementsAdmin);
document.addEventListener('DOMContentLoaded', () => {
  const domElement = document.getElementById('GenericElementsAdmin');
  if (domElement) ReactDOM.render(<GenericElementsAdminDnD />, domElement);
});
